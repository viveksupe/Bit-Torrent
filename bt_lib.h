...
typedef struct {
  ...
  struct sockaddr_in bindToThis;
  int pCheck;       //for leeder
...
void calc_Info_Hash(FILE * f);
void leecher(peer_t* peer, bt_args_t* bt_args,
		char* msg);
void seeder(peer_t* peer, bt_args_t* bt_args,
		char* msg);
// to compare hash
int compareInfoHash(char* handshakeMessage, bt_args_t * bt);
int compareIdHash(char * msg, int port, char * ip);
void getBitFieldMessage(char * bitMessage);
void prepareInterestedMessage (bt_msg_t * btmsg,char * msg);
void prepareBitFieldMessage(bt_msg_t * bt,char * msg,bt_args_t* bt_args);
};
void prepareRequestMsg(char * data,char*msg,int index,int begin, int length);
void sendRequestedData(bt_args_t* bt_args, int index, int begin,int nbytes,char *data);
void storeInFile(char * dataToFile,int indexInFile,int offsetInFile,bt_args_t * bt,FILE* f);
